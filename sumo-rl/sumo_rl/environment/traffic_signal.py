...
class TrafficSignal:
    ...
    def _balance_queues_reward(self):
        """Rewards for balanced queues across all lanes."""
        queues = self.get_lanes_queue()
        # Calculate the standard deviation of queue lengths to measure balance
        mean_queue = np.mean(queues)
        queue_balance = np.std(queues)
        reward = -(queue_balance)  # Reward is higher for lower standard deviation
        return reward
    
    def _improved_flow_reward(self):
        """Calculates the reward based on the improvement in the flow of traffic."""
        current_flow = self.get_average_speed()
        reward = current_flow - self.last_measure
        self.last_measure = current_flow
        return reward
    
    def _avg_waiting_time_reward(self):
        """Calculates the reward based on the decrease in average waiting time per vehicle."""
        total_vehicles = sum(self.sumo.lane.getLastStepVehicleNumber(lane) for lane in self.lanes)
        if total_vehicles == 0:
            return 0
        avg_waiting_time = sum(self.get_accumulated_waiting_time_per_lane()) / total_vehicles
        reward = self.last_measure - avg_waiting_time
        self.last_measure = avg_waiting_time
        return reward
    
    def _exponential_wait_time_punishment(self):
        """Calculates a reward that exponentially punishes long waiting times in each lane."""
        wait_times = self.get_accumulated_waiting_time_per_lane()
        total_exponential_penalty = sum(np.exp(wait_time / 100) for wait_time in wait_times)  # Adjust the divisor to scale the exponential growth
        reward = -total_exponential_penalty  # Negative reward (punishment)
        return reward
    
    def _lane_imbalance_punishment(self):
        """Calculates a reward that severely punishes imbalances in vehicle counts across lanes."""
        vehicle_counts = np.array([self.sumo.lane.getLastStepVehicleNumber(lane) for lane in self.lanes])
        variance_sq = np.var(np.square(vehicle_counts))  # Calculate the variance of vehicle counts across lanes
        
        reward = -(variance_sq + sum(vehicle_counts)) # Use variance to scale the punishment
        return reward
    
    def _wait_lane_combine(self):
        return self._exponential_wait_time_punishment() \
            + self._lane_imbalance_punishment()


    reward_fns = {
        "diff-waiting-time": _diff_waiting_time_reward,
        "avg-speed": _average_speed_reward,
        "queue": _queue_reward,
        "pressure": _pressure_reward,

        "avg-wait-time": _avg_waiting_time_reward,
        "improved-flow": _improved_flow_reward,
        "balance-queues": _balance_queues_reward,

        "exp-wait-time": _exponential_wait_time_punishment,
        "lane-equal" : _lane_imbalance_punishment,
        "wait-lane" : _wait_lane_combine,
    }
